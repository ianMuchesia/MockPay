...
const SubscribePage: React.FC = () => {
  ...
  const { data: plansData, isLoading: loadingPlans, isError: errorPlans } = useGetSubscriptionPlansQuery();
  const { data: servicesData, isLoading: loadingServices, isError: errorServices } = useGetServicesQuery();

  const [createSubscription, { isLoading: creatingSubscription, isSuccess: subscriptionSuccess, isError: subscriptionError, error: subscriptionErrMsg }] = useCreateSubscriptionMutation();
  ...
  useEffect(() => {
    ...
    if (subscriptionError) {
      const errMsg = subscriptionErrMsg && 'data' in subscriptionErrMsg ? (subscriptionErrMsg.data as any).message : 'Failed to create subscription.';
  ...
  const selectedService = servicesData?.data.find(service => service.id === selectedServiceId);
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4 mb-6">
        <div className="flex-grow">
          <h1 className="text-2xl font-bold text-neutral-800">Subscribe to Business</h1>
      ...
      <Card className="mb-6">
        ...
        <div className="flex items-center gap-4 mb-8 pb-6 border-b border-neutral-100">
          ...
          <div>
            <h2 className="text-xl font-bold">Business ID: {businessId?.substring(0, 8)}...</h2>
            <p className="text-neutral-500">Select subscription options below</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            {plansData?.data.length === 0 && (
              <div className="col-span-3 text-center py-10">
                ...
                <h3 className="text-xl font-medium text-neutral-700">No subscription plans available</h3>
                <p className="text-neutral-500 mt-2">Try checking out the standalone services</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            {servicesData?.data.length === 0 && (
              <div className="col-span-3 text-center py-10">
                ...
                <h3 className="text-xl font-medium text-neutral-700">No standalone services available</h3>
                <p className="text-neutral-500 mt-2">Try selecting a subscription plan instead</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row border border-neutral-100 rounded-lg p-4 bg-neutral-50 mb-6">
            <div className="flex-grow">
              <div className="flex items-center">
                <i className={`fas ${selectedPlan ? 'fa-cube' : 'fa-cog'} text-primary mr-2`}></i>
              ...
              {selectedPlan && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-neutral-500">Includes Services</p>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{selectedPlan.durationInDays} days</p>
                  </div>
                </div>
              )}
              ...
              {selectedService && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm font-medium">{selectedService.type}</p>
                  </div>
                  <div>
                    <p className="text-sm text-neutral-500">Duration</p>
                    <p className="text-sm font-medium">{selectedService.standaloneDurationInDays} days</p>
                  </div>
                </div>
              )}
            </div>
      ...
      {!selectedPlan && !selectedService && (
        <div className="text-center mt-8">
          <p className="text-neutral-500 mb-4">Please select a subscription plan or service to continue</p>
          <Button
            disabled={true}
            isFullWidth
          >
